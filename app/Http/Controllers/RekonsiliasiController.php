    ...
    public function get_data(Request $request)
    {
        ...
        foreach ($list_wilayah as $id_wil => $wilayah) {
            ...
            foreach ($periode_filter as $periode) {
                ...
                $adhb_c = Rekon::select('kode_kab', 'tahun', 'q', 'adhb_or_adhk', $kolom, $kolom_adj)
                    ->where('kode_kab', $id_wil)
                    ->where('tahun', $arr_periode[0])
                    ->wherein('q', $q_c)
                    ->where('adhb_or_adhk', 1)
                ...
                $adhb_c1 = Rekon::select('kode_kab', 'tahun', 'q', 'adhb_or_adhk', $kolom, $kolom_adj)
                    ->where('kode_kab', $id_wil)
                    ->where('tahun', $arr_periode[0] - 1)
                    ->wherein('q', $q_c)
                    ->where('adhb_or_adhk', 1)
                ...
                $row[$periode . '_adhb_c'] = $adhb_c ? $adhb_c->$kolom : null;
                $row[$periode . '_adhb_c1'] = $adhb_c1 ? $adhb_c1->$kolom : null;
