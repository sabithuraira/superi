            ...
            <div class="card">
                <div class="header">
                    ...
                    <div class="alert alert-info">
                        <ul>
                            <li>Menu rekonsiliasi berfungsi sebagai alat untuk melakukan rekonsiliasi seluruh kabupaten/kota</li>
                            <li>Isian input dapat menghitung secara otomatis nilai pertumbuhan dan nilai implisit</li>
                            <li>Fungsi Tombol Toogle Implisit berubah(Sebelah Kanan)</li>
                            <ul>
                                <li>toogle pilihan Implisit Berubah(adhb tetap) dan Implisit Tetap(adhb berubah)</li>
                                <li><b>Implisit Tetap</b> akan berfungsi ketika menginput nilai pertumbuhan bukan nilai komponennya</li>
                                <li><b>Implisit Berubah(adhb tetap)</b>: ketika input pada kolom pertumubuhan(qtq,yty,cyc) maka akan mengubah nilai adj
                                    adhk tanpa mengubah nilai adhb</li>
                            </ul>
                            <li>Tombol simpan akan melakukan proses simpan nilai adj ke server, tapi apabila admin approve nilai baru maka adj dari 0 lagi
                            </li>
                            <li>Tombol export untuk download data dalam XLS tapi tidak menyimpan data ke server</li>
                        </ul>
                    </div>
                </div>
                <div class="body">
                            ...
                            <form id="form_filter">
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-sm-6 col-md-1 ">
                                        ...
                                        <button class="btn btn-info w-100 text-wrap" type="button" id="simpan" @click="saveDatas">Simpan</button>
                                    </div>
                                    <div class="form-group col-sm-6 col-md-1 ">
                                        <label class="text-white">Export</label>
                                        <button class="btn btn-success w-100 text-wrap" type="button" id="export" onclick="exportHtmlTableToExcel()">Export
                                            <i class="fa fa-download"></i></button>
                                    </div>
                                    <div class="form-group col-sm-6 col-md-2 ">
                                        <label class="text-white">Create Excel</label>
                                        <button class="btn btn-success w-100 text-wrap" style="white-space: normal" type="button" id="create" onclick="createhasilrekon()">create hasil rekon
                                            <i class="fa fa-download"></i></button>
                                    </div>
...
@section('scripts')
    ...
    <script>
                    ...
                    $.ajax({
                        ...
                        self.periode_filter.forEach(periode => {
                            ...
                            periode_header += '<th colspan=18 class="fixed-row-1 ' + color_header + '">' + periode + '</th>';
                               ...
                               if(triwulan==4){
                                periode_header+='<th colspan=12 class="fixed-row-1 ' + color_header + '">' + periode.substring(0,4) + '</th>';
                               }
                        ...
                        data.data.forEach(row => {
                            ...
                            self.periode_filter.forEach(periode => {
                                ...
                                triwulan = periode.substring(5, 6)
                                if(triwulan==4){
                                    const adhb_c = row[periode + '_adhb_c'] !== null && row[periode + '_adhb_c'] !== undefined ?
                                        parseFloat(row[periode + '_adhb_c']) : "";
                                    const adhb_c_adj = row[periode + '_adhb_c_adj'] !== null && row[periode + '_adhb_c_adj'] !==
                                        undefined ? parseFloat(row[periode + '_adhb_c_adj']) : 0;

                                    const adhb_c1 = row[periode + '_adhb_c1'] !== null && row[periode + '_adhb_c1'] !==
                                        undefined ? parseFloat(row[periode + '_adhb_c1']) : "";
                                    const adhb_c1_adj = row[periode + '_adhb_c1_adj'] !== null && row[periode +
                                        '_adhb_c1_adj'] !== undefined ? parseFloat(row[periode + '_adhb_c1_adj']) : "";

                                    const implisit_ctc = adhb_c != "" && adhk_c != "" ? (adhb_c / adhk_c * 100) : "";
                                    const implisit_ctc_adj = adhk_c + adhk_c_adj != "" ?
                                        ((adhb_c + adhb_c_adj) / (adhk_c + adhk_c_adj) * 100) : "";
                                    const laju_implisit_ctc = adhb_c != "" && adhk_c != "" && adhb_c1 != "" && adhk_c1 != "" ?
                                        ((adhb_c / adhk_c * 100) / (adhb_c1 / adhk_c1 * 100) * 100 - 100) : "";
                                    const laju_implisit_ctc_adj =
                                        adhk_c + adhk_c_adj != "" &&
                                        adhk_c1 + adhk_c1_adj != "" ?
                                        (((adhb_c + adhb_c_adj) / (adhk_c + adhk_c_adj) * 100) /
                                        ((adhb_c1 + adhb_c1_adj) / (adhk_c1 + adhk_c1_adj) * 100) * 100) - 100 : "";
                                    rowHtml += `
                                    <td data-adhb_c="${adhb_c}"
                                        data-adhb_c_adj="${adhb_c_adj}"

                                        data-adhb_c1="${adhb_c1}"
                                        data-adhb_c1_adj="${adhb_c1_adj}">
                                        ${formatNumber(adhb_c)}
                                    </td>
                                    ${(String(row['kode_kab'] || '')).startsWith('d') ? '<td></td>'
                                    :  `<td>
                                        <input id="${periode + '_'+ row['kode_kab'] + '_adhb_c_adj'}"
                                        value="${formatNumber(adhb_c_adj)}"
                                        data-id="${row[periode + '_adhb_c_id']}"
                                        data-kode_kab="${row['kode_kab']}"
                                        data-periode="${periode}"
                                        class="text_edit_adhb ${row['kode_kab']} text-right"
                                        type="text" inputmode="decimal"
                                        pattern="^\d+(\.\d{0,2})?$"
                                        ${row[periode+'_adhb_c_id'] == null?'disabled':''}
                                        >
                                    </td>
                                        `}
                                    ${(String(row['kode_kab'] || '')).startsWith('d%')
                                        ? `<td id="cell_adhb_adj_${periode}" data-kode_kab="${row['kode_kab']}" style="${adhb_c_adj > 2 ? 'background:orangered' : adhb_c_adj < -2 ? 'background:orangered' : ''}"> ${formatNumber(adhb_c_adj)} </td>`
                                        : `<td id="cell_adhb_adj_${periode}" data-kode_kab="${row['kode_kab']}"> ${formatNumber(adhb_c + adhb_c_adj)} </td>`
                                    }
                                    <td
                                        data-adhk_c="${adhk_c}"
                                        data-adhk_c_adj="${adhk_c_adj}"
                                    ...
                                    :`
                                        <td>
                                            <input id="${periode+ '_'+ row['kode_kab'] + '_adhk_c_adj'}"
                                            value="${formatNumber(adhk_c_adj)}"
                                            data-id="${row[periode + '_adhk_c_id']}"
                                            data-kode_kab="${row['kode_kab']}"
                                            data-periode="${periode}"
                                            class="text_edit_adhk text-right" type="text"
                                            inputmode="decimal" pattern="^\d+(\.\d{0,2})?$"
                                            ${row[periode+'_adhk_c_id'] == null?'disabled':''}
                                            >
                                        </td>`
                                    }
                                    ${(String(row['kode_kab'] || '')).startsWith('d%')
                                        ? `<td id="cell_adhk_adj_${periode}" data-kode_kab="${row['kode_kab']}" style="${adhk_c_adj > 2 ? 'background:orangered' : adhk_c_adj < -2 ? 'background:orangered' : ''}"> ${formatNumber(adhk_c_adj)} </td>`
                                        : `<td id="cell_adhk_adj_${periode}" data-kode_kab="${row['kode_kab']}"> ${formatNumber(adhk_c + adhk_c_adj)} </td>`
                                    }


                                    ${(String(row['kode_kab'] || '')).startsWith('d') ? '<td></td><td></td><td></td><td></td><td></td><td></td>'
                                    :  `<td style="${ctc > 0 ? 'background:lightgreen' : ctc < 0 ? 'background:lemonchiffon' : ''}">${formatNumber(ctc)}</td>
                                        <td style="${ctc_adj > 0 ? 'background:lightgreen' : ctc_adj < 0 ? 'background:lemonchiffon' : ''}" >${formatNumber(ctc_adj)}</td>
                                        <td style="${implisit_ctc > 0 ? 'background:lightgreen' : implisit_ctc < 0 ? 'background:lemonchiffon' : ''}">${formatNumber(implisit_ctc)}</td>
                                        <td style="${implisit_ctc_adj > 0 ? 'background:lightgreen' : implisit_ctc_adj < 0 ? 'background:lemonchiffon' : ''}">${formatNumber(implisit_ctc_adj)}</td>
                                        <td style="${laju_implisit_ctc > 0 ? 'background:lightgreen' : laju_implisit_ctc < 0 ? 'background:lemonchiffon' : ''}">${formatNumber(laju_implisit_ctc)}</td>
                                        <td style="${laju_implisit_ctc_adj > 0 ? 'background:lightgreen' : laju_implisit_ctc_adj < 0 ? 'background:lemonchiffon' : ''}">${formatNumber(laju_implisit_ctc_adj)}</td>
                                        `}
                                `;
                                }
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (response.success) {
                                ...
                                swal("Error!", response.message || "Data gagal disimpan", "error");
                            }
        ...
        $(document).on('input', '.text_edit_adhb', function() {
            ...
            const kode_kab = input.data('kode_kab');
            ...
            // ADHB Q1
            let input_adhb_q1 = $('#' + str_q1 + "_" + kode_kab + '_adhb_adj');
            ...
            // ADHB Y1
            let input_adhb_y1 = $('#' + (year - 1) + 'Q' + quarter + "_" + kode_kab + '_adhb_adj')
            ...
            // ADHK Q1
            let input_adhk_q1 = $('#' + str_q1 + "_" + kode_kab + '_adhk_adj');
            ...
            // ADHK Y1
            let input_adhk_y1 = $('#' + (year - 1) + 'Q' + quarter + "_" + kode_kab + '_adhk_adj')
            ...
            totalkabkotadhb(periode)
            hitungdiskreadhb(periode)
        ...
        $(document).on('input', '.text_edit_adhk', function() {
            ...
            // ADHK Q1
            let input_adhk_q1 = $('#' + str_q1 + "_" + kode_kab + '_adhk_adj');
            // console.log('#' + str_q1 + "_" + kode_kab + '_adhk_adj')
            // console.log(input_adhk_q1.data())
            if (input_adhk_q1.length > 0 && input_adhk_q1.is('input')) {
                // jika input ada pada tampilan
                adhk_q1_adj = parseNumberIndonesian(input_adhk_q1.val());
            } else {
                // jika input tidak ada ambil dari attribute data
                adhk_q1_adj = parseNumberIndonesian(adhk_cell.data('adhk_q1_adj'));
            }
            ...
            for (var q_c = 1; q_c <= quarter; q_c++) {
                ...
                let input_adhk_c = $('#' + str_c + "_" + kode_kab + '_adhk_adj')
                if (input_adhk_c.length > 0 && input_adhk_c.is('input')) {
                    // console.log('isinput' + input_adhk_c.val())
                ...
                let input_adhk_c1 = $('#' + str_c1 + "_" + kode_kab + '_adhk_adj')
            ...
            totalkabkotadhk(periode)
            hitungdiskreadhk(periode)
            formatedtext = formatText(input.val());
            $(this).val(formatedtext);
        });
        ...
        $(document).on('input', '.text_edit_qtq_adj', function() {
            ...
            let input_adhb_q1 = $('#' + str_q1 + '_' + kode_kab + '_adhk_adj');
            ...
            let input_adhk_y1 = $('#' + str_y1 + '_' + kode_kab + '_adhk_adj');
            if (input_adhk_y1.length > 0 && input_adhk_y1.is('input')) {
                adhk_y1_adj = parseNumberIndonesian(input_adhk_q1.val());
            } else {
                adhk_y1_adj = parseNumberIndonesian(adhk_cell.data('adhk_y1_adj'));
            }
            let input_adhb_y1 = $('#' + str_y1 + '_' + kode_kab + '_adhk_adj');
            if (input_adhb_y1.length > 0 && input_adhb_y1.is('input')) {
                adhb_y1_adj = parseNumberIndonesian(input_adhb_y1.val());
            } else {
                adhb_y1_adj = parseNumberIndonesian(adhb_cell.data('adhb_y1_adj'));
        ...
        $(document).on('input', '.text_edit_yty_adj', function() {
            ...
            let input_adhb_y1 = $('#' + str_y1 + '_' + kode_kab + '_adhb_adj');
            if (input_adhb_y1.length > 0 && input_adhb_y1.is('input')) {
                adhb_y1_adj = parseNumberIndonesian(input_adhb_y1.val());
            } else {
                adhb_y1_adj = parseNumberIndonesian(adhb_cell.data('adhb_y1_adj'));
        ...
        function totalkabkotadhb(periode) {
            let sum = 0;
            $(".text_edit_adhb").each(function() {
                let kode = $(this).data("kode_kab");
                let periode_input = $(this).data("periode");
                if (kode !== "-" && kode !== "00" && periode === periode_input) {
                    sum += parseNumberIndonesian($(this).val()) || 0;
                }
            });
            // $(".text_edit_adhb[data-kode_kab='-'][data-periode='" + periode + "']").val(formatNumber(sum));

            let q_split = periode.split('Q');
            let year = parseInt(q_split[0]);
            let quarter = parseInt(q_split[1]);

            const input = $(".text_edit_adhb[data-kode_kab='-'][data-periode='" + periode + "']")
            input.val(formatNumber(sum));
            const adhb_adj = sum
            const adhk_adj = parseNumberIndonesian(input.closest('td').nextAll().eq(2).find('input').val() || 0);
        ...
        function totalkabkotadhk(periode) {
            let sum = 0;
            $(".text_edit_adhk").each(function() {
                let kode = $(this).data("kode_kab");
                let periode_input = $(this).data("periode");
                if (kode !== "-" && kode !== "00" && periode === periode_input) {
                    sum += parseNumberIndonesian($(this).val()) || 0;
            ...
            const input = $(".text_edit_adhk[data-kode_kab='-'][data-periode='" + periode + "']")
            input.val(formatNumber(sum));
            const kode_kab = input.data('kode_kab')
            const adhk_adj = sum;
            const adhb_adj = parseNumberIndonesian(input.closest('td').prevAll().eq(2).find('input').val() || 0);
