                                    ...
                                    <tbody>
                                            ...
                                            <tr style="@if ($shouldBold) background-color:#f2f2f2; font-weight: bold; @endif">
                                                ...
                                                @foreach ($periode_filter as $periode)
                                                    @php
                                                        ...
                                                        $hk = $dt[$periode . 'adhk'];
                                                    @endphp
                                                    <td class="text-right" style="@if (abs($hb - $hk) > 5) background-color : yellow; @endif">
                                        ...
                                        <tr>
                                            ...
                                            @foreach ($periode_filter as $periode)
                                                <td class="text-right" style="@if (abs($hb - $hk) > 5) background-color : yellow; @endif">
                                                    @php
                                                        $komp_ekspor = collect($data)->firstWhere('komponen', 'c_6');
                                                        $komp_impor = collect($data)->firstWhere('komponen', 'c_7');
                                                    @endphp

                                                    {{ $komp_ekspor && $komp_impor && array_key_exists($periode . 'adhb', $komp_ekspor) && array_key_exists($periode . 'adhb', $komp_impor)
                                                        ? number_format(round($komp_ekspor[$periode . 'adhb'] - $komp_impor[$periode . 'adhb'], 2), 2, ',', '.')
                                                        : '' }}
                                                </td>
                                                <td class="text-right" style="@if (abs($hb - $hk) > 5) background-color : yellow; @endif">
                                                    {{ $komp_ekspor && $komp_impor && array_key_exists($periode . 'adhk', $komp_ekspor) && array_key_exists($periode . 'adhk', $komp_impor)
                                                        ? number_format(round($komp_ekspor[$periode . 'adhk'] - $komp_impor[$periode . 'adhk'], 2), 2, ',', '.')
                                                        : '' }}
                                                </td>
                                            @endforeach
                                        </tr>
