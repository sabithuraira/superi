    ...
    public function get(Request $request) {
        ...
        foreach ($c_all as $c) {
            ...
            if ($tabel == 'Tabel 2.1') {
                ...
                foreach ($periode as $p) {
                    $pembagi_prov = DB::select('SELECT SUM(IF(CONCAT(tahun, "Q", q) = "' . $p . '" AND kode_prov = "16" AND kode_kab = "00" AND adhb_or_adhk = 1, ' . $c . ', 0)) AS prov FROM superi_pdrb')[0]->prov;
                ...
                foreach ($periode as $p) {
                    ...
                    $q = substr($p, 5, 1);
                    if ($q == '1') {
                        $pembagi_prov = DB::select('SELECT SUM(IF(tahun = ' . ($tahun - 1) . ' AND q = 4 AND kode_prov = "16" AND kode_kab = "00" AND adhb_or_adhk = 2, c_pdrb, 0)) AS prov FROM superi_pdrb')[0]->prov;
                        ...
                        $pembagi_prov = DB::select('SELECT SUM(IF(tahun = ' . $tahun . ' AND q = ' . ($q - 1) . ' AND kode_prov = "16" AND kode_kab = "00" AND adhb_or_adhk = 2, c_pdrb, 0)) AS prov FROM superi_pdrb')[0]->prov;
                ...
                foreach ($periode as $p) {
                    $tahun = substr($p, 0, 4);
                    $q = substr($p, 5, 1);
                    $pembagi_prov = DB::select('SELECT SUM(IF(tahun = ' . ($tahun - 1) . ' AND q = ' . $q . ' AND kode_prov = "16" AND kode_kab = "00" AND adhb_or_adhk = 2, c_pdrb, 0)) AS prov FROM superi_pdrb')[0]->prov;
